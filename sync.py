...
def compile_claude(config: dict, project: dict, guardrails_block: str, base_dir: Path) -> dict:
...
"""
    return {claude_file: claude_content}

def compile_gemini(config: dict, project: dict, guardrails_block: str, base_dir: Path) -> dict:
    """Compiles AGENTS.md and GEMINI.md for Google Gemini / Antigravity. Returns map of file paths to contents."""
...
def generate_all_outputs(base_dir: Path, provider_override: str = None) -> dict:
    ...
    if provider in ("all", "claude"):
        all_outputs.update(compile_claude(config, project, guardrails_block, base_dir))
...
def run_sync(base_dir: Path, provider_override: str = None):
    ...
    for fpath, content in outputs.items():
        ...
        if ".codex" in str(fpath):
            ...
            claude_updated = True
        elif fpath.name in ("AGENTS.md", "GEMINI.md"):
            gemini_updated = True
    ...
    if gemini_updated:
        print("  ✓ Gemini / Antigravity: Generated AGENTS.md and GEMINI.md")
        
    print("✓ Synchronization complete.\n")

def run_check(base_dir: Path, provider_override: str = None) -> int:
    """Checks if current generated files match config.toml. Returns 0 if matching, 1 otherwise."""
    outputs = generate_all_outputs(base_dir, provider_override)
    mismatched = []
    missing = []
    ...
    if not missing and not mismatched:
        print("✓ All provider configurations are in sync with config.toml and agents/.")
        return 0
    else:
...
def run_init(target_dir: Path):
    """Initializes config.toml via stack auto-detection if missing, then compiles provider files."""
    config_path = target_dir / "config.toml"
    print(f"Initializing Autonomous Multi-Agent Protocol in: {target_dir.resolve()}")
    
    if config_path.exists():
        print(f"  ℹ Existing config.toml found in {target_dir.resolve()}. Preserving settings.")
    else:
        ...
        print(f"  ✓ Generated tailored config.toml for {stack_info.get('name')}")
...
def main():
    ...
    parser.add_argument("--init", nargs="?", const=".", default=None, metavar="DIR",
                        help="Initialize config.toml with auto-detected stack in target directory (defaults to current dir) and compile.")
    parser.add_argument("--check", action="store_true",
                        help="Check whether generated provider files are in sync with config.toml (exits with 0 if up-to-date, 1 if out-of-sync).")
    parser.add_argument("--provider", choices=["all", "codex", "claude", "gemini"], default=None,
                        help="Override active provider (all, codex, claude, or gemini).")
    parser.add_argument("--dir", default=None, metavar="DIR",
                        help="Directory containing config.toml and agents/ (defaults to script directory).")
