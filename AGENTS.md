...
- **Generated Assets Rule**: Never spawn an LLM agent to copy, diff, or patch generated build assets. Use deterministic build commands (`npm run build`).
...
- **Workflow:** `quick-implementer` → runs focused unit check (`npm test -- {file}`) → complete.
...
- **Validate New Risk, Not Re-Run Proof**: If the implementer verified focused unit tests to green, DO NOT re-run that command in `code-validator`. `code-validator` runs ONLY broader suites (`npm run test:integration`) or builds (`npm run build`).
