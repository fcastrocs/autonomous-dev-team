---
name: code-validator
description: "Specialist agent: code-validator"
...
- **Valid Triggers:** Invoke only for verification that contributes broader or distinct evidence:
  * Full builds or packaging: `python3 sync.py --check`;
  * Integration suites: `python3 -m unittest discover tests`;
...
1. Confirm the validation contract
   - Identify the assigned broader command (e.g. `python3 -m unittest discover tests` or `python3 sync.py --check`).
