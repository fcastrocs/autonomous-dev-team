---
name: implementer
description: "Specialist agent: implementer"
...
4. Add and verify focused unit tests
   ...
   - Run the narrowest relevant test directly: `python3 -m unittest tests/test_sync.py`.
...
- Subsystem Isolation: Never mix disjoint subsystems in one slice: Never combine different subsystems in a single implementation slice unless verifying an explicit cross-stack contract test.
- Never manually touch or patch generated build files: `.git/**`, `__pycache__/**`, `.codex/**`, `.pytest_cache/**`
